...
int main(int argc, char **argv)
{
	...
	getOpt(argc, argv, &s, &E, &b, &verbose, tracefile);
...
void find(char op, unsigned addr, unsigned size, int time) {
	...
	block *cache_set = cache + E * set_index ;
	block *eviction_block = cache_set;
	for(i = 0; i < E; i++) {
		if(cache_set[i].usedtime>0 && cache_set[i].tag ==tag) {
			cache_set[i].usedtime = time;
			hit++;
			if(verbose) cacheStateOut(op,0);
			return;
		} else if(!cache_set[i].usedtime) {
			miss++;
			cache_set[i].tag = tag;
			cache_set[i].usedtime = time;
			if(verbose) cacheStateOut(op,1);
			return;
        } else if(cache_set[i].usedtime < eviction_block->usedtime) {
